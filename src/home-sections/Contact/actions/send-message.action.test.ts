...
describe('sendMessage', () => {
  ...
  beforeEach(() => {
    ...
    accept.mockResolvedValue({ kind: EContactSubmissionAcceptanceKind.accepted, messageId: 42 })
    notify.mockResolvedValue(undefined)
  })

  it('persists normalized input and acknowledges its public submission ID', async () => {
    await expect(
      sendMessage({ status: EContactSubmissionStatus.idle }, formData()),
    ).resolves.toEqual({
  ...
  it('rejects non-FormData input with a stable form error', async () => {
    ...
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, malformed)).toEqual({
      status: EContactSubmissionStatus.validationError,
  ...
  it('normalizes omitted optional fields to empty strings', async () => {
    ...
    await sendMessage({ status: EContactSubmissionStatus.idle }, data)
    expect(accept).toHaveBeenCalledWith(
  ...
  it('rejects unknown and duplicate fields', async () => {
    ...
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, unknown)).toEqual({
      status: EContactSubmissionStatus.validationError,
      fieldErrors: { form: ['invalid_type'] },
    })
    ...
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, duplicate)).toEqual({
      status: EContactSubmissionStatus.validationError,
  ...
  it.each([
    ...
    expect(
      await sendMessage({ status: EContactSubmissionStatus.idle }, formData({ attachments })),
    ).toEqual({
      status: EContactSubmissionStatus.validationError,
  ...
  it('passes one decoded attachment reference to persistence', async () => {
    ...
    await sendMessage(
      { status: EContactSubmissionStatus.idle },
      formData({ attachments: JSON.stringify([item]) }),
    )
  ...
  it('returns the original success for a replay without another mail claim', async () => {
    accept.mockResolvedValue({ kind: EContactSubmissionAcceptanceKind.replay, messageId: 42 })
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, formData())).toEqual({
      status: EContactSubmissionStatus.success,
      submissionId,
    })
    expect(notify).not.toHaveBeenCalled()
  ...
  it('rejects payload changes on an accepted submission ID', async () => {
    accept.mockResolvedValue({ kind: EContactSubmissionAcceptanceKind.mismatch })
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, formData())).toEqual({
      status: EContactSubmissionStatus.validationError,
  ...
  it('maps authoritative attachment rejection to the attachment field', async () => {
    accept.mockResolvedValue({ kind: EContactSubmissionAcceptanceKind.invalidAttachments })
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, formData())).toEqual({
      status: EContactSubmissionStatus.validationError,
      fieldErrors: { attachments: ['attachment_invalid'] },
    })
  ...
  it('fails closed with sanitized unavailable state for identity or database outages', async () => {
    ...
    const result = await sendMessage({ status: EContactSubmissionStatus.idle }, formData())
    expect(result).toEqual({
      status: EContactSubmissionStatus.unavailable,
      code: 'service_unavailable',
    })
  ...
  it('reports rate limiting and keeps SMTP failure from changing stored success', async () => {
    accept.mockResolvedValueOnce({ kind: EContactSubmissionAcceptanceKind.rateLimited })
    expect(await sendMessage({ status: EContactSubmissionStatus.idle }, formData())).toEqual({
      status: EContactSubmissionStatus.rateLimited,
      code: 'rate_limited',
    })

    accept.mockResolvedValueOnce({ kind: EContactSubmissionAcceptanceKind.accepted, messageId: 43 })
